...
let ApartmentRepository = class ApartmentRepository extends typeorm_1.Repository {
    constructor(dataSource) {
        super(apartment_entity_1.ApartmentEntity, dataSource.createEntityManager());
    }
    async getByApartment(apartment) {
        return await this.findOne({ where: { apartment } });
    }
    async getById(id) {
        return await this.findOne({ where: { id } });
    }
    async createApartment(newApartment) {
        return new Promise(async (resolve, reject) => {
            try {
                const apartment = new apartment_entity_1.ApartmentEntity();
                const dataApartment = Object.assign(Object.assign({}, apartment), newApartment);
